...
# Busca archivos Excel SOLO en Prueba_Tecnica\RPA
def find_excel_files():
    log_message("Buscando archivos .xlsx...")

    # Ruta del script actual:
    # Prueba_Tecnica\Framework\PIX\script
    script_dir = os.path.dirname(os.path.abspath(__file__))

    # Subimos 3 niveles:
    # script -> PIX -> Framework -> Prueba_Tecnica
    base_dir = os.path.abspath(os.path.join(script_dir, "..", "..", ".."))

    # Ruta final donde están los excels
    rpa_dir = os.path.join(base_dir, "RPA")

    log_message(f"Directorio script: {script_dir}")
    log_message(f"Directorio base detectado: {base_dir}")
    log_message(f"Ruta RPA detectada: {rpa_dir}")

    pattern = os.path.join(rpa_dir, "*.xlsx")
    ...
    if files:
        log_message(f"Excel encontrados: {files}")
    else:
        log_message("No se encontraron archivos Excel en la carpeta RPA.")
...
def run_full_process():
    ...
    if not reportes:
        log_message("No se encontró ningún archivo con formato válido tipo Reporte_YYYY-MM-DD.xlsx")
        return False
    ...
    log_message(f"Archivo más reciente detectado: {archivo_mas_reciente}")

    token = get_access_token_device_code()
    if not token:
        log_message("No se pudo obtener token.")
        return False

    if not test_onedrive_connection(token):
        log_message("Falló la conexión a OneDrive.")
        return False
...
def main():
    ...
    log_message(f"Directorio actual (cwd): {os.getcwd()}")
    try:
        run_full_process()
    except Exception as e:
